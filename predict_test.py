...
class YOLOEvaluator:
    ...
    def comprehensive_evaluation(self, split="test"):
        ...
        self.metrics = self.model.val(
            data=self.data_yaml, split=split, save_json=True, plots=True, conf=0.001, iou=0.6, device=0
        )
    ...
    def _print_class_metrics(self):
        ...
        if hasattr(self.metrics.box, "aps") and self.metrics.box.aps is not None:
            ...
            # 按AP值排序
            sorted_aps = sorted(enumerate(self.metrics.box.aps), key=lambda x: x[1], reverse=True)
...
def main():
    # 配置
    evaluator = YOLOEvaluator(model_path="runs/detect/train3/weights/best.pt", data_yaml="datasets/dataset.yaml")

    # 执行全面评估
    evaluator.comprehensive_evaluation(split="test")
